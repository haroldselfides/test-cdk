  ...
  constructor(scope, id, props) {
   ...
   const employeeNotificationsDLQ = new sqs.Queue(this, 'EmployeeNotificationsDLQ', {
    ...
    const lambdaEnvironment = {
      ...
      HR_ADMIN_EMAIL_FROM: process.env.HR_ADMIN_EMAIL_FROM, // ✅ ADD THIS
    ...
    const testEmployeeStreamProcessorLambda = new NodejsFunction(this, 'TestEmployeeStreamProcessorLambda', functionProps('../lambda/notifications/streamProcessor.js'));    
    ...
    // Grant permissions for SES notification lambda
    ...
    // Add SQS event source for SES lambda
    ...
    employeeNotificationsQueue.grantConsumeMessages(sesAdminNotificationLambda);

    sesAdminNotificationLambda.addToRolePolicy(new iam.PolicyStatement({
      actions: ['ses:SendEmail', 'ses:SendRawEmail'],
      resources: ['*'], // or limit to your verified FROM email
    }));

    sesAdminNotificationLambda.addEventSource(new lambdaEventSources.SqsEventSource(employeeNotificationsQueue));
    ...
    employeeNotificationsQueue.grantSendMessages(employeeUpdateStreamProcessorLambda);
    ...
    new CfnOutput(this, 'NotificationQueueUrl', {
      ...
      description: 'URL of the Employee Notification Queue',
    });

    new CfnOutput(this, 'NotificationQueueArn', {
      value: employeeNotificationsQueue.queueArn,
      description: 'ARN of the Employee Notification Queue',
    });

    new CfnOutput(this, 'NotificationDLQUrl', {
      value: employeeNotificationsDLQ.queueUrl,
      description: 'URL of the Employee Notification Dead Letter Queue',
    });
