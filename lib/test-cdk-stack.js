...
class TestCdkStack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    updateEmployeeLambda.addToRolePolicy(new iam.PolicyStatement({
      actions: [
        "dynamodb:UpdateItem",
    ...
    deleteEmployeeLambda.addToRolePolicy(new iam.PolicyStatement({
      actions: [
        "dynamodb:UpdateItem",
        "dynamodb:ConditionCheckItem"
      ],
    ...
    testTable.grantReadWriteData(updateEmployeeLambda);
    testTable.grant(updateEmployeeLambda, 'dynamodb:UpdateItem');
    testTable.grant(deleteEmployeeLambda, 'dynamodb:UpdateItem');
    testTable.grant(updateEmployeeLambda, 'dynamodb:PutItem');
    testTable.grant(getEmployeeDetailsLambda, 'dynamodb:Query');

    testTable.grant(getPersonalDataLambda, 'dynamodb:GetItem');
    testTable.grant(updatePersonalDataLambda, 'dynamodb:UpdateItem');
    testTable.grant(getContactInfoLambda, 'dynamodb:GetItem');
    testTable.grant(updateContactInfoLambda, 'dynamodb:UpdateItem');
    testTable.grant(getContractDetailsLambda, 'dynamodb:GetItem');
    testTable.grant(updateContractDetailsLambda, 'dynamodb:UpdateItem');
